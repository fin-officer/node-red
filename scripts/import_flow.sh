#!/bin/bash
# Skrypt do importu flow Node-RED
...
echo -e "${BLUE}[INFO]${NC} Importowanie flow do Node-RED..."
...
cat > /tmp/email-llm-flow.json << 'EOF'
[
    {
        "id": "email-llm-flow",
        "type": "tab",
...
# Kopiuj plik flow do kontenera
docker cp /tmp/email-llm-flow.json nodered-email-llm:/data/email-llm-flow.json

echo -e "${BLUE}[INFO]${NC} Flow zostało skopiowane do kontenera Node-RED."
...
echo -e "4. Kliknij 'wybierz plik do zaimportowania' i wybierz plik /data/email-llm-flow.json"
echo -e "5. Kliknij 'Import'"
echo ""
echo -e "${YELLOW}[INFO]${NC} Alternatywnie możesz otworzyć plik w przeglądarce, skopiować jego zawartość i wkleić przez Import w Node-RED."
